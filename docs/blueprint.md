...
- 각 인스턴스는 서명 가능 기간이 있는 Ed25519 키쌍을 생성한다. 개인키는 메모리에만 두고 공개키를 별도 Redis에 TTL과 함께 등록한다. 백그라운드 작업이 만료 전 교체하며 등록 실패에는 같은 키로 계속 재시도한다. `/readyz`와 요청은 키 등록을 유발하지 않는다. 공개키는 구 대기표가 만료될 때까지 유지하며 키 Redis 장애에는 서명 필요 발급·갱신과 검증을 닫는다. 공개키 등록 권한은 인프라 수준에서 제한한다.
