...
| `GET /readyz` | 대기열 Redis와 installation marker, 등록된 현재 서명키, MySQL 준비 여부. 읽기 전용이며 키 등록은 유발하지 않음 |
...
대기표 버전 2는 서버가 생성한 Ed25519 키로 서명하며 `kid`로 별도 Redis의 공개키를 찾습니다. 키가 아직 등록되지 않았거나 키 Redis가 불가하면 서명이 필요한 대기표 발급·갱신은 `TICKET_KEY_UNAVAILABLE`(503, 재시도 가능)을 반환합니다. 키 미등록 상태는 새 대기 순번을 소비하지 않고, 서명이 필요 없는 직접 입장은 가능합니다. 알 수 없는 `kid`와 이전 HMAC 버전 1 대기표는 `TICKET_INVALID`입니다.
