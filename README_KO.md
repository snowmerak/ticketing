...
장기 보관하는 개인키는 `memguard` 잠긴 메모리에 두고 교체·종료 시 파기합니다. 서명 중에는 지우는 일반 Go 메모리 사본을 잠깐 사용하므로 엔클레이브나 프로세스 권한 공격 방어를 뜻하지 않습니다. 메모리 잠금 한도는 [설정 참조 문서](./docs/configuration.md)를 보세요.
